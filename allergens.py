"""
Allergen detection module.

detect_allergens(ingredients) takes a flat list of ingredient names
(ingredients + sub-ingredients) and returns display names, e.g.
["Milk", "Wheat/Gluten"].

The system uses:
    1. Explicit ingredient matching (keyword rules)
    2. ML-based prediction as supporting evidence

The product name itself is not used for allergen detection.

Requires vectorizer.joblib and model.joblib in the same folder as this file.
"""
...
def detect_explicit_allergens(ingredients: list[str]) -> set[str]:
    detected = set()
    ...
    for ingredient in ingredients:
        ...
        for allergen, keywords in EXPLICIT_ALLERGENS.items():
            for keyword in keywords:
                # .lower() so keywords like "TVP" can actually match,
                # because the text is already lowercased above.
...
def detect_allergens(ingredients: list[str]) -> list[str]:
    """
    Takes a flat list of ingredient names (ingredients + sub-ingredients)
    and returns allergen display names in a consistent order.
    """
    ...
    # "peanut butter" -> Peanuts, "milk powder" -> Milk, "wheat flour" -> Wheat/Gluten
    detected = detect_explicit_allergens(ingredients)

    # STEP 2 — ML supporting detection
    # Only add an ML prediction when the probability reaches ML_THRESHOLD.
    text = ". ".join(ingredients)
    X = vectorizer.transform([text])
    probabilities = model.predict_proba(X)[0]

    for allergen, probability in zip(ALLERGEN_COLUMNS, probabilities):
